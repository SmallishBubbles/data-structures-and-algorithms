...
const firstLetters = (arr) => {
  return arr.map(stringy => stringy.charAt(0));
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(number => number.split(/[()\- ]/).join(''));
};
...
const onlyOddChars = (str) => {
  let arr = str.split('');
  let filtered = arr.filter((item, idx) => {
    if (idx % 2 === 1) {
      return true;
    }
  });
...
const unenrollBrook = (arr) => {
  return arr.map(studentArr => studentArr.filter(student => {
